...
복잡해짐 — "게임플레이/설정/게임 방법/기록" 4개 버튼만 있는 메인 메뉴를 새로 만들고
기존 내용을 하위 화면으로 재배치. 시작 전에 확인한 것: 메인 메뉴 신설(기존 화면은 하위
패널로), 음악/효과음 각각 볼륨 슬라이더(저장됨), 게임 방법 화면은 정적 텍스트.

### 화면 구조 (`StartScreen.cs`, 전면 재작성)
- `Panel` enum(MainMenu/Play/Settings/HowToPlay/Leaderboard) 하나로 어느 화면이 떠
  있는지 관리 — `SwitchTo(Panel)`가 다섯 그룹 GameObject 중 하나만 활성화. 기존
  `showLeaderboard` bool + `baseContent` 단일 화면 구조를 대체
- **메인 메뉴**: 타이틀 + Best 점수 + 4개 버튼(게임플레이/설정/게임 방법/기록)
- **게임플레이 화면**: 기존 StartScreen의 대부분(닉네임 입력+재생성, 코인, 새 선택 줄+알
  구매, 오늘의 미션 패널) + 뒤로가기 버튼. "빈 곳 탭하면 시작" 캐처(`TapCatcher`)를 캔버스
  루트가 아니라 **이 화면 그룹의 자식**으로 옮김 — 그룹이 비활성화되면 캐처도 같이
  없어져서 별도 상태 체크 없이 "이 화면에서만 탭-시작 동작"이 자연히 성립함(버튼이 캐처보다
  나중 형제로 그려져서 클릭을 먼저 가로채는 원리는 기존과 동일). 스페이스바 시작도
  `currentPanel == Panel.Play`일 때만 폴링하도록 가드 추가
- **설정 화면**: 음악/효과음 볼륨 슬라이더, 언어 전환 버튼(기존 위치에서 이동), 로그인/
  로그아웃 버튼(기존 위치에서 이동 — 로그인/회원가입 폼은 여기서 열림), 뒤로가기
- **게임 방법 화면**: 정적 설명 텍스트(조작법 + Great Slide/Fever/코인/구름/섬 배수 규칙
  요약, 한/영 둘 다 `Localization.cs`의 `howtoplay.body` 키에 있음), 뒤로가기
- **기록(리더보드) 화면**: 기존 패널 그대로 재사용, 여닫히는 지점만 메인 메뉴 기준으로
  변경("닫기" → `SwitchTo(Panel.MainMenu)`)
- 로그인/회원가입 폼(`authFormGroup`)은 이 5단계 상태 머신과 별개로 설정 화면 위에 뜨는
  모달 — 기존 구조 그대로, 여는 위치만 설정 화면으로 이동
...
9. **온라인 계정/랭킹**: 🚧 Phase A(백엔드)+B(네트워킹/로그인 UI) 완료 (2026-08-18), **Phase
   ...
   확인할 것.
10. **시작 화면 메인 메뉴 재구성**: ✅ 구현 완료 (2026-08-18). 메인 메뉴(게임플레이/설정/
    게임 방법/기록) + 4개 하위 화면, 음악/효과음 볼륨 슬라이더(저장됨). **플레이 확인
    전혀 안 됨** — 특히 메인 메뉴 각 버튼 → 하위 화면 → 뒤로가기 왕복, 게임플레이 화면
    에서만 탭/스페이스바로 런이 시작되는지(다른 화면에서는 안 되는지), 볼륨 슬라이더가
    실제로 소리 크기를 바꾸고 재시작 후에도 유지되는지 확인 필요.
...
- [ ] 틀린 비밀번호/중복 아이디로 로그인·회원가입 시도 시 서버 에러 메시지가 폼에 그대로
      뜨는지 (온라인 계정 Phase B)
- [ ] 시작 화면이 메인 메뉴(게임플레이/설정/게임 방법/기록 4개 버튼)로 뜨는지, 각 버튼 →
      하위 화면 → 뒤로가기로 메인 메뉴 복귀가 전부 되는지 (메인 메뉴 재구성)
- [ ] **게임플레이 화면에서만 빈 곳 탭/클릭/스페이스바로 런이 시작되는지 — 메인 메뉴/설정/
      게임 방법/기록 화면에서는 절대 시작되지 않는지** (메인 메뉴 재구성 — tap catcher를
      게임플레이 화면 그룹 안으로 옮긴 것 검증)
...
1. **시작 화면 — 메인 메뉴**: 타이틀 + Best 점수 + 4개 버튼(게임플레이/설정/게임 방법/기록)
   - **게임플레이**: 좌상단 닉네임 입력 필드(직접 수정 가능) + "재생성" 버튼, 우상단 코인
     총액 + 뒤로가기, 좌하단 오늘의 미션 패널, 하단 보유 새 선택 줄(클릭해서 선택) + 알
     구매 버튼(500코인, 부화 결과 토스트 표시). **이 화면에서만** 버튼이 아닌 빈 곳을
     터치/클릭하거나 스페이스바를 누르면 시작(닉네임/아이디/비밀번호 입력 필드에 포커스가
     있을 땐 스페이스바로 시작되지 않음) — 메인 메뉴/설정/게임 방법/기록 화면에서는 탭해도
     시작되지 않음
   - **설정**: 음악/효과음 볼륨 슬라이더(저장됨), 언어 전환 버튼(한/영, 지금 언어가 아니라
     눌렀을 때 바뀔 언어를 표시), 로그인/로그아웃 버튼(로그인 상태면 "{서버 닉네임} ·
     로그아웃"으로 표시 — 게임플레이 화면의 로컬 닉네임과는 다른 별개의 온라인 계정
     닉네임), 뒤로가기
   - **게임 방법**: 조작법 + 핵심 규칙 요약 정적 텍스트, 뒤로가기
   - **기록**: Top 10 + 누적 통계 패널, 닫기(메인 메뉴로 복귀)
